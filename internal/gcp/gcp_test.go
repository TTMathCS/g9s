...
func TestResourceRowsMatchColumns(t *testing.T) {
	fixtures := map[string]Resource{
		...
		"artifacts":  repositoryResource(testProject(), "us-central1", testRepository()),
		"redis":      redisResource(testProject(), testRedisInstance()),
		"bigtable":   bigtableResource(testProject(), testBigtableInstance()),
		"bigtableclusters": bigtableClusterResource(testProject(), "events-store",
			testBigtableCluster()),
		"firestore":  firestoreResource(testProject(), testFirestoreDatabase()),
		"datastream": streamResource(testProject(), testStream()),
		"datafusion": dataFusionResource(testProject(), testDataFusionInstance()),
		"bqreservations": bqReservationResource(testProject(), "US",
			testBQReservation()),
		"memcached":  memcacheResource(testProject(), testMemcacheInstance()),
		"spanner":    spannerInstanceResource(testProject(), testSpannerInstance()),
		"spannerdbs": spannerDatabaseResource(testProject(), "orders-prod", testSpannerDatabase()),
		...
		"alerts":    alertPolicyResource(testProject(), testAlertPolicy()),
		...
		"runexecs": executionResource(testProject(),
			runJobResource(testProject(), "us-central1", testCloudRunJob()), testCloudRunExecution()),
		"topicsubs": topicSubscriptionResource(testProject(), testPubSubSubscription(),
