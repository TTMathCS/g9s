...
| BigQuery jobs | global | jobs are project-global with the location on each row; scoped by `defaults.bigquery_job_window`, which defines the listing rather than truncating it, and then `limits.bigquery_jobs` (default 500), which does truncate it and says so |
| Dataproc clusters | **regional** | needs a client per region at `<region>-dataproc.googleapis.com`; `global` always swept |
| Dataproc jobs | **regional** | same per-region clients as the clusters; every state, newest first, `limits.dataproc_jobs_per_region` (default 200) because the API has no time filter |
...
| Load balancer backend health | a forwarding rule | the expensive one, and the argument for the whole mechanism: rule → target proxy → URL map → every backend service it routes to → `getHealth` per backend group. Four-plus round trips to answer for one row, which nobody would pay on every refresh of the load balancers table and everybody would pay once, on the rule they are actually looking at. `limits.backend_groups` (default 40) bounds *requests* rather than rows, so raising it costs latency |
...
| DNS record sets | a zone | one paginated call, `limits.dns_record_sets` (default 1000). Grouped by name rather than sorted flat, so a name's A and AAAA sit on adjacent rows — they are read as a group |
