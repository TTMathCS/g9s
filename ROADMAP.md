...
| Cloud Storage buckets | global | 1 | the project inventory stays cheap: one call lists buckets, while their potentially enormous object namespaces are fetched only after a bucket is opened. On the generated REST client rather than `cloud.google.com/go/storage`: the hand-written wrapper was a second client stack with its own transport, auth path and release cadence, and the one dependency here whose version had to line up with `google.golang.org/api` rather than simply being it. Dropping it removed eighteen transitive dependencies — the whole OpenTelemetry SDK and metric exporter among them — and put storage failures through the same `*googleapi.Error` classification as every other kind. `projection=full` on the listing keeps the lifecycle rules free rather than costing a Get per bucket |
